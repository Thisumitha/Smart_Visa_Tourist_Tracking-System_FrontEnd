...
import { ShieldCheck, UserPlus, Globe, History, Book, FileText, LogOut, LayoutDashboard } from 'lucide-react';

interface ImmigrationSidebarProps {
    activeTab: string;
    onTabChange: (tab: string) => void;
}

const ImmigrationSidebar: React.FC<ImmigrationSidebarProps> = ({ activeTab, onTabChange }) => {
    ...
    const navItems = [
        { id: 'dashboard', label: 'Profiling Dashboard', icon: <LayoutDashboard size={20} />, activeColor: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' },
        { id: 'wizard', label: 'New Registration', icon: <UserPlus size={20} />, activeColor: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' },
    ...
    return (
        <aside className="w-64 glass-panel border-y-0 border-l-0 rounded-none flex flex-col hidden md:flex">
            <div className="p-6 border-b border-glassborder flex items-center gap-3">
                ...
                <span className="font-bold text-lg tracking-wide text-white">Immigration Control</span>
            </div>
